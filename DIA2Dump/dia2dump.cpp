...
bool ParseArg(int argc, wchar_t *argv[])
{
  ...
  else if (!_wcsicmp(argv[0], L"-id")) {

	  iCount = 1;
	  bReturn = bReturn && DumpId(g_pDiaSession, g_pGlobalSymbol);
	  argc -= iCount;
	  bReturn = bReturn && ParseArg(argc, &argv[iCount]);
  }
...
void PrintHelpOptions()
{
  static const wchar_t * const helpString = L"usage: Dia2Dump.exe [ options ] <filename>\n"
                                            ...
                                            L"  -id               : print debug and code identifier\n"
...
static bool
ReadPE(const wchar_t *filename, std::wstring &code_file, DWORD &timestamp, DWORD &imageSize)
{
	char *namebuf;
	size_t bufsize = wcslen(filename) * sizeof(wchar_t);
	size_t c;

	namebuf = new char[bufsize];
	if (wcstombs_s(&c, namebuf, bufsize, filename, bufsize) != 0) {
		delete[] namebuf;
		return false;
	}
	PLOADED_IMAGE img = ImageLoad(namebuf, NULL);
	if (!img) {
		delete[] namebuf;
		return false;
	}
	...
	timestamp = img->FileHeader->FileHeader.TimeDateStamp;
	...
	if (opt->Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
		...
		// 32-bit PE file.
		imageSize = img->FileHeader->OptionalHeader.SizeOfImage;
	}
	delete[] namebuf;
	return true;
}
...
////////////////////////////////////////////////////////////
// Dump debug identifier(guid+age) in PDB
//  plus code identifier(timestamp+size) in EXE/DLL
bool DumpId(IDiaSession *pSession, IDiaSymbol *pGlobal)
{
	GUID guid;
	DWORD age;
	BSTR pdbname;
	...
	if (FAILED(pGlobal->get_symbolsFileName(&pdbname)))
		return false;

	if (FAILED(pGlobal->get_guid(&guid)))
		return false;

	if (FAILED(pGlobal->get_age(&age)))
		return false;

	// Use the same format that the MS symbol server uses in filesystem
	// hierarchies.
	wchar_t age_string[9];
	swprintf(age_string, sizeof(age_string) / sizeof(age_string[0]), L"%x", age);
	...
	wprintf(L"%s,%s%s\n", pdbname, guid_string, age_string);

	std::wstring code_file;
	DWORD timestamp, imageSize;
	if (SUCCEEDED(pGlobal->get_timeStamp(&timestamp)) &&
		ReadPE(g_szFilename, code_file, timestamp, imageSize)) {
		wprintf(L"%s,%08X%X\n", code_file.c_str(), timestamp, imageSize);
	}
